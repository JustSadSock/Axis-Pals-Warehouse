...
Кооперативная top-down головоломка про двух квадратов, которые двигаются вместе: один игрок отвечает за горизонталь, второй — за вертикаль. Если одному мешает стена, второй всё равно сдвинется, поэтому взаимное положение постоянно меняется. Работает локально на одной клавиатуре или онлайн через WebSocket-сервер (готов к запуску за Cloudflare Tunnel).
...
## Управление
- Игрок 1: **A / D** или стрелки **← →** — сдвигает оба объекта по горизонтали.
- Игрок 2: **W / S** или стрелки **↑ ↓** — сдвигает оба по вертикали.
...
- Цель каждого уровня — поставить оба квадрата на подсвеченные точки.
...
## Логика
- `public/gameLogic.js` — единый модуль для сервера и клиента (проверки стен, одновременное движение, победа).
- `public/levels.js` — 5 уровней с акцентом на синхронные оси.
